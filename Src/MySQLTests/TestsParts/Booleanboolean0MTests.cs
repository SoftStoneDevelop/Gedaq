...
ORDER BY
    m.id ASC
";
                foreach(var batchResult in await ((IBooleanSingleTypeboolean)this).DbConnectionDynQuerySelectModelBatchAsync(connection, 4, query1, 68, query2))
                {
                    if(++resultIndex == 1)
                    {
                        var models = batchResult;
...
ORDER BY
    m.id ASC
";
                foreach(var batchResult in  ((IBooleanSingleTypeboolean)this).DbConnectionDynQuerySelectModelBatch(connection, 7, query1, 16, query2))
                {
                    if(++resultIndex == 1)
                    {
                        var models = batchResult;
...
Gedaq.DbConnection.Attributes.BatchPart(
        ...
        public async Task DbConnectionSelectModelBatchTestAsync()
        {
            await using (var connection = GlobalSetUp.GetDbConnection)
            {
                await connection.OpenAsync();
                int resultIndex = 0;
                foreach(var batchResult in await ((IBooleanSingleTypeboolean)this).DbConnectionSelectModelBatchAsync(connection, 96, 140))
                {
                    if(++resultIndex == 1)
                    {
                        var models = batchResult;
...
Booleanboolean0M.AssertModel(models[0],_testData[28], false);Booleanboolean0M.AssertModel(models[1],_testData[29], false);
        ...
        public async Task DbConnectionSelectModelBatchTest()
        {
            await using (var connection = GlobalSetUp.GetDbConnection)
            {
                await connection.OpenAsync();
                int resultIndex = 0;
                foreach(var batchResult in  ((IBooleanSingleTypeboolean)this).DbConnectionSelectModelBatch(connection, 36, 125))
                {
                    if(++resultIndex == 1)
                    {
                        var models = batchResult;
