...
");
            ...
            )
        {
            ...
            {
                builder.Append($@"        
        {source.AccessModifier.ToLowerInvariant()} {source.MethodStaticModifier} {QueryCommonBase.GetScalarTypeName(source, ProviderInfo)} Scalar{source.MethodName}(
");
            }
            else
            {
                builder.Append($@"        
        {source.AccessModifier.ToLowerInvariant()} {source.MethodStaticModifier} async Task<{QueryCommonBase.GetScalarTypeName(source, ProviderInfo)}> Scalar{source.MethodName}Async(
");
            }
        }
